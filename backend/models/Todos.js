...
class Todo {
  constructor({ todo_id, todo_title, todo_description, is_finished, user_id }) {
    this.todoId = todo_id;
  ...
  static async getAllByUserId(id) {
    const response = await db.query("SELECT * FROM todos WHERE user_id = $1", [
      id,
    ]);
  ...
  static async getCompletedByUserId(id) {
    const response = await db.query(
      "SELECT * FROM todos WHERE user_id = $1 AND is_finished = true",
      [id]
    );
    if (response.rows.length == 0) {
  ...
  static async getIncompletedByUserId(id) {
    const response = await db.query(
      "SELECT * FROM todos WHERE user_id = $1 AND is_finished = false",
      [id]
    );
    if (response.rows.length == 0) {
  ...
  static async getByTodoId(id) {
    const response = await db.query("SELECT * FROM todos WHERE todo_id = $1", [
      id,
    ]);
